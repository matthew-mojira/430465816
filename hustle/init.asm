...
incsrc "registers.asm"  ; snes register defines

ORG $C00000

incsrc "types.asm"    ; type system definitions
incsrc "ram.asm"      ; the runtime's usage of RAM and other definitions

incsrc "main.asm"     ; runtime system
incsrc "err.asm"      ; error handling
incsrc "io.asm"       ; I/O and v-blank code

GraphicsData: incbin !GraphicsData
TilemapData:  incbin !TilemapData

code_begin:
incsrc "42.asm"     ; compiled code TODO
code_end:

; debug information regarding how big your program is
print "Compiled code usage: ", \
  dec(code_end-code_begin), \
  " bytes of maximum ", \
  dec($C08000-code_begin)

warnpc $C08000-1      ; ensure ROM size does not clobber interrupt area
...
ORG $00FFE0
    ...
    dw $0000, $0000, $0000, I_BRK, $0000, I_NMI, I_RESET, I_IRQ      ; NATIVE
    dw $0000, $0000, $0000, I_BRK, $0000, I_NMI, I_RESET, I_IRQ      ; EMULATION
...
F_RESET:
    ...
    JSL main    ; we won't be coming back
    STP         ; SToP the clock

I_NMI:
    LDA !RDNMI  ; read for NMI acknowledge
    JSL vblank
